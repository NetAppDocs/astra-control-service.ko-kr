...
클러스터가 정상 상태이며 지원되는 Kubernetes 버전을 실행 중인지, 작업자 노드가 온라인 상태이고 Linux 또는 Windows가 실행 중인지 등을 확인합니다. <<EKS cluster requirements,이 단계에 대해 자세히 알아보십시오>>.
...
[role="quick-margin-para"]
아마존 계정이 없는 경우 EKS를 사용할 수 있도록 계정을 만들어야 합니다. <<Create an Amazon account,이 단계에 대해 자세히 알아보십시오>>.
...
[role="quick-margin-para"]
명령줄에서 AWS를 관리할 수 있도록 AWS CLI를 설치합니다. <<Install the Amazon Web Services CLI,단계별 지침을 따릅니다>>.
...
[role="quick-margin-para"]
<<Optional: Create an IAM user,단계별 지침을 읽습니다>>.
...
[role="quick-margin-para"]
<<Create and attach a permissions policy,단계별 지침을 읽습니다>>.
...
[role="quick-margin-para"]
<<Save the credentials for the IAM user,단계별 지침을 읽습니다>>.
