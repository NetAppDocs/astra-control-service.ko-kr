...
Astra Control은 Kubernetes 네이티브 애플리케이션을 위한 애플리케이션 데이터 라이프사이클 관리 운영을 간소화하고자 합니다. Astra Control Service는 GKE(Google Kubernetes Engine) 및 AKS(Azure Kubernetes Service)에서 실행되는 Kubernetes 클러스터를 지원합니다.
...
예. GKE 클러스터에서 Google Cloud용 Cloud Volumes Service 또는 Azure NetApp Files의 AKS 클러스터에서 사용하는 영구 볼륨에 대해 비용이 청구됩니다.
...
아니요 클러스터가 Astra Control에 추가되면 서비스가 Kubernetes 클러스터에 NetApp의 Trident Container Storage Interface(CSI) 드라이버를 자동으로 설치합니다. 이 CSI 드라이버는 Google Cloud용 NetApp Cloud Volumes Service 및 Azure NetApp Files에서 지원하는 AKS 클러스터에 영구 볼륨을 프로비저닝하는 데 사용됩니다.
...
예, Astra Control Service에 개인 GKE 클러스터를 추가할 수 있습니다. GKE(Google Kubernetes Engine) 프라이빗 클러스터를 생성하려면 https://kb.netapp.com/Advice_and_Troubleshooting/Cloud_Services/Project_Astra/How_to_create_a_private_GKE_cluster_to_work_with_project_Astra["이 기술 자료 문서의 지침을 따르십시오"^].
...
Astra Trident는 Astra Control에서 제거할 때 클러스터에서 제거되지 않습니다.
...
* 내 애플리케이션이 검색된 앱 목록에 표시되지 않습니다. 문제를 식별하기 위해 무엇을 확인할 수 있습니까? *

애플리케이션이 * Discovered Apps * 에 나열되지 않으면 'kubtl get pod-A | grep [pod name]'을 실행하여 Kubernetes Pod의 상태와 상태를 확인하십시오. Pod가 정상 실행 중이면 * Ignored Apps * 아래에 응용 프로그램이 나열되어 있는지 확인합니다.
