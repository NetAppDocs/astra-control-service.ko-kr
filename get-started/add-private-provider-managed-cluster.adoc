...
Azure Kubernetes Service 및 Google Kubernetes Engine 클러스터를 관리할 때 Astra Trident 설치 및 라이프사이클 관리에 대한 두 가지 옵션이 있습니다.

* Astra Control Service를 사용하여 Astra Trident의 라이프사이클을 자동으로 관리할 수 있습니다. 이렇게 하려면 Astra Control Service로 관리할 클러스터에 Astra Trident가 설치되어 있지 않은지 확인하십시오. 이 경우 클러스터 관리를 시작할 때 Astra Trident가 Astra Trident를 자동으로 설치하며 Astra Trident 업그레이드는 자동으로 처리됩니다.
* Astra Trident의 라이프사이클을 직접 관리할 수 있습니다. 이렇게 하려면 Astra Control Service로 클러스터를 관리하기 전에 클러스터에 Astra Trident를 설치합니다. 이 경우 Astra Control Service는 Astra Trident가 이미 설치되어 있음을 감지하며 해당 Trident를 다시 설치하거나 Astra Trident 업그레이드를 관리하지 않습니다. 을 참조하십시오 https://docs.netapp.com/us-en/trident/trident-get-started/kubernetes-deploy.html["Astra Trident 문서"^] 를 참조하십시오.


Astra Control Service를 통해 Amazon Web Services 클러스터를 관리하는 경우 Astra Trident를 통해 지원되는 스토리지 백엔드가 필요한 경우 Astra Control Service를 통해 클러스터를 관리하기 전에 클러스터에 Astra Trident를 수동으로 설치해야 합니다. 을 참조하십시오 https://docs.netapp.com/us-en/trident/trident-get-started/kubernetes-deploy.html["Astra Trident 문서"^] 를 참조하십시오.
...
====
.단계
. (선택 사항) Amazon EKS 클러스터를 추가하거나 Astra Trident의 설치 및 업그레이드를 직접 관리하려는 경우 클러스터에 Astra Trident를 설치합니다. 을 참조하십시오 https://docs.netapp.com/us-en/trident/trident-get-started/kubernetes-deploy.html["Astra Trident 문서"^] 를 참조하십시오.
