...
* https://["프라이빗 AKS 클러스터를 위한 Azure 설명서"^]
* https://["전용 OpenShift 클러스터를 위한 Azure 문서"^]
...
----
. 의 지침에 따라 Astra Control API 토큰을 생성합니다 https://["Astra 자동화 문서"^].
. 를 포함하도록 Astra Connector 운영자 리포지토리의 config/samples 디렉토리에서 예제 구성 파일을 수정합니다 `spec.natssync-client.cloud-bridge-url` 키. 예를 들면 다음과 같습니다.
+
[listing]
----
...
* https://["Azure NetApp Files를 사용하여 Microsoft Azure를 설정합니다"]
* https://["Azure 관리 디스크를 사용하여 Microsoft Azure를 설정합니다"]
...
이제 Astra Control Service에 프라이빗 클러스터를 추가할 수 있습니다. 표준 워크플로우를 따라 클러스터를 Astra Control Service에 추가합니다. https://["Astra Control Service에서 Kubernetes 클러스터 관리를 시작합니다"].
